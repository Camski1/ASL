<div id="nav">

	<a href="http://localhost:8888/cameron_kozinski"><img src="http://localhost:8888/cameron_kozinski/img/logo.svg"></a>
 
	<ul>
		<a href="http://localhost:8888/cameron_kozinski"><li>Home</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/us"><li>U.S</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/world"><li>World</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/sciTech"><li>Sci/Tech</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/politics"><li>Politics</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/travel"><li>Travel</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/sports"><li>Sports</li></a>
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/life"><li>Life</li></a>
	</ul>
	<div id="navRight">
		<a href="http://localhost:8888/cameron_kozinski/index.php/Aysi_news/join">Sign In | Sign Up</a>

		<form>
			<input id="Search" type="text" name="Search" placeholder="Search..." >
			<input id="searchItem" type="submit" value="Search" />
		</form>
	</div>
</div>